            ...
            <TabsContent value="menu">
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-lg font-semibold text-[#E8DDB5]">Custom Navigation Items</h2>
                    <p className="text-sm text-gray-500">Add custom links to the sidebar navigation</p>
                  </div>
                  <Button onClick={() => { resetNavForm(); setNavDialogOpen(true); }} className="bg-[#D4A836] hover:bg-[#C49A30] text-black">
                ...
                {navItems.length === 0 ? (
                  <Card className="bg-[#0f0f15] border-[#D4A836]/20">
                    <CardContent className="py-12 text-center">
                      ...
                      <p className="text-gray-400">No custom menu items yet</p>
                      <p className="text-sm text-gray-500 mt-1">Add links to internal pages, external URLs, or CMS pages</p>
                    </CardContent>
                  </Card>
                ) : (
                  <div className="card-dark rounded-xl border border-[#D4A836]/20 overflow-hidden">
                    <Table>
                      <TableHeader>
                        <TableRow className="border-[#D4A836]/20">
                          ...
                          <TableHead className="text-gray-400">Visible To</TableHead>
                          <TableHead className="text-gray-400">Status</TableHead>
                          <TableHead className="text-gray-400 text-right">Actions</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {navItems.map((item) => (
                          <TableRow key={item.item_id} className="border-[#D4A836]/10">
                            <TableCell className="text-[#E8DDB5] font-medium">{item.label}</TableCell>
                            <TableCell>
                              <Badge variant="outline" className="border-[#D4A836]/30 text-[#D4A836]">
                                {item.link_type === 'internal' ? 'Internal' : item.link_type === 'external' ? 'External' : 'CMS Page'}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-gray-400 text-sm max-w-[200px] truncate">{item.path}</TableCell>
                            <TableCell className="text-gray-400">{SECTION_OPTIONS.find(s => s.id === item.section_id)?.label || item.section_id}</TableCell>
                            <TableCell>
                              <div className="flex flex-wrap gap-1">
                                {(item.visible_to || ['all']).map(role => (
                                  <Badge key={role} variant="secondary" className="text-xs bg-[#1a1a24] text-gray-400">
                            ...
                            <TableCell className="text-right">
                              <Button size="sm" variant="ghost" onClick={() => editNavItem(item)} className="text-gray-400 hover:text-[#E8DDB5]">
                                <Edit className="w-4 h-4" />
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => deleteNavItem(item.item_id)} className="text-gray-400 hover:text-red-400">
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </TableCell>
                          </TableRow>
                ...
                <Dialog open={navDialogOpen} onOpenChange={setNavDialogOpen}>
                  <DialogContent className="bg-[#0f0f15] border-[#D4A836]/20 max-w-xl">
                    <DialogHeader>
                      <DialogTitle className="text-[#E8DDB5]">{editingNavItem ? 'Edit Menu Item' : 'Add Menu Item'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleNavSubmit} className="space-y-4 mt-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                        ...
                        <div>
                          ...
                          <Select value={navForm.link_type} onValueChange={(v) => setNavForm({...navForm, link_type: v, path: ''})}>
                            <SelectTrigger className="bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="bg-[#1a1a24] border-[#D4A836]/30">
                      ...
                      <div>
                        <Label className="text-gray-400">
                          {navForm.link_type === 'internal' ? 'Path (e.g., /dashboard)' : 
                           navForm.link_type === 'external' ? 'External URL' : 'Select CMS Page'}
                        </Label>
                        {navForm.link_type === 'cms_page' ? (
                          <Select value={navForm.path} onValueChange={(v) => setNavForm({...navForm, path: v})}>
                            <SelectTrigger className="bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]">
                              <SelectValue placeholder="Select a page" />
                            </SelectTrigger>
                            <SelectContent className="bg-[#1a1a24] border-[#D4A836]/30">
                              {cmsPages.map(page => (
                                <SelectItem key={page.id} value={`/page/${page.slug}`}>{page.title}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        ) : (
                          <Input 
                            ...
                            placeholder={navForm.link_type === 'internal' ? '/my-page' : 'https://example.com'}
                            className="bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]" 
                            required 
                          />
                        )}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Select value={navForm.section_id} onValueChange={(v) => setNavForm({...navForm, section_id: v})}>
                            <SelectTrigger className="bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="bg-[#1a1a24] border-[#D4A836]/30">
                        ...
                        <div>
                          ...
                          <Select value={navForm.icon} onValueChange={(v) => setNavForm({...navForm, icon: v})}>
                            <SelectTrigger className="bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="bg-[#1a1a24] border-[#D4A836]/30 max-h-60">
                      ...
                      <div>
                        <Label className="text-gray-400 mb-2 block">Visible To (select roles)</Label>
                        <div className="flex flex-wrap gap-2">
                          {ROLE_OPTIONS.map(role => (
                            <button
                              key={role.id}
                              type="button"
                              onClick={() => toggleNavItemRole(role.id)}
                              className={`px-3 py-1 rounded-full text-sm border transition-colors ${
                                navForm.visible_to?.includes(role.id)
                                  ? 'bg-[#D4A836]/20 border-[#D4A836] text-[#D4A836]'
                                  : 'bg-[#1a1a24] border-[#30363D] text-gray-400 hover:border-[#D4A836]/50'
                              }`}
                            >
                              {role.label}
                            </button>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-[#30363D]">
                        <div className="flex items-center gap-4">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => setNavForm({...navForm, open_in_new_tab: e.target.checked})}
                              className="w-4 h-4"
                            />
                            <span className="text-gray-400 text-sm">Open in new tab</span>
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="checkbox"
                        ...
                        <div className="flex items-center gap-2">
                          <Label className="text-gray-400 text-sm">Sort Order:</Label>
                          <Input
                            type="number"
                            value={navForm.sort_order}
                            onChange={(e) => setNavForm({...navForm, sort_order: parseInt(e.target.value) || 100})}
                            className="w-20 bg-[#1a1a24] border-[#D4A836]/30 text-[#E8DDB5]"
                          />
                        </div>
                      </div>

                      <div className="flex justify-end gap-3">
                        <Button type="button" variant="outline" onClick={() => setNavDialogOpen(false)} className="border-[#D4A836]/30 text-[#E8DDB5]">Cancel</Button>
                        <Button type="submit" disabled={saving} className="bg-[#D4A836] hover:bg-[#C49A30] text-black">
                          {saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
