...
export default function ModuleBuilder() {
  ...
  const handleBulkImport = async () => {
    if (!bulkPreview || bulkPreview.length === 0) { toast.error('No valid modules'); return; }
    setBulkUploading(true);
    try {
      const res = await axios.post(API + '/training/modules/bulk', { modules: bulkPreview }, { headers: { Authorization: 'Bearer ' + token } });
      toast.success(res.data.message || 'Import successful');
      setBulkJson(''); setBulkPreview(null); setBulkError('');
      const modsRes = await axios.get(API + '/training/modules', { headers: { Authorization: 'Bearer ' + token } });
      setModules(modsRes.data);
    } catch (err) {
      toast.error((err.response && err.response.data && err.response.data.detail) || 'Bulk import failed');
  ...
  const handleExportModules = async () => {
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'modules_export.json'; a.click();
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 space-y-8" data-testid="module-builder-page">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button onClick={handleExportModules} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#30363D] text-gray-300 hover:text-white text-sm transition-all">
            <Download className="w-4 h-4" />Export All
          </button>
        </div>

        <div className="flex gap-1 p-1 rounded-xl bg-[#0d1117] border border-[#30363D] w-fit">
          <button onClick={() => setActiveTab('designer')} className={'flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ' + (activeTab === 'designer' ? 'bg-[#2979FF] text-white shadow' : 'text-gray-400 hover:text-white')}>
            <Code className="w-4 h-4" />Designer
          </button>
          <button onClick={() => setActiveTab('bulk-import')} className={'flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ' + (activeTab === 'bulk-import' ? 'bg-[#2979FF] text-white shadow' : 'text-gray-400 hover:text-white')}>
        ...
        {activeTab === 'bulk-import' && (
          <div className="space-y-6">
            <Card className="bg-[#161B22] border-[#30363D]">
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2"><FileJson className="w-5 h-5 text-[#2979FF]" />Bulk Import Modules</CardTitle>
                <p className="text-sm text-gray-400 mt-1">Paste JSON or upload a file. Accepts a single module, an array, or a modules object. Duplicates are skipped.</p>
              </CardHeader>
              <CardContent className="space-y-5">
                <div onClick={() => fileInputRef.current && fileInputRef.current.click()} className="border-2 border-dashed border-[#30363D] hover:border-[#2979FF]/60 rounded-xl p-10 text-center cursor-pointer transition-all group">
                  ...
                  <input ref={fileInputRef} type="file" accept=".json,application/json" className="hidden" onChange={handleBulkFileSelect} />
                </div>
                <div className="flex items-center gap-3"><div className="flex-1 h-px bg-[#30363D]" /><span className="text-xs text-gray-500 uppercase tracking-widest">or paste JSON</span><div className="flex-1 h-px bg-[#30363D]" /></div>
                <textarea value={bulkJson} onChange={(e) => handleBulkJsonChange(e.target.value)} rows={12} spellCheck={false} placeholder='[{"name":"Phishing Awareness","module_type":"phishing","difficulty":"easy","duration_minutes":15,"is_active":true}]' className="w-full rounded-lg bg-[#0d1117] border border-[#30363D] text-sm text-gray-200 font-mono px-4 py-3 focus:outline-none focus:border-[#2979FF]/60 resize-y placeholder-gray-600" />
                {bulkError && (<div className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-3"><AlertTriangle className="w-4 h-4 shrink-0" />{bulkError}</div>)}
                {bulkPreview && bulkPreview.length > 0 && (
                  <div className="rounded-xl border border-[#30363D] overflow-hidden">
                    <div className="flex items-center gap-2 px-4 py-3 bg-[#21262D] border-b border-[#30363D]">
                      ...
                      <span className="text-sm font-medium text-white">{bulkPreview.length} module{bulkPreview.length !== 1 ? 's' : ''} ready to import</span>
                    </div>
                      ...
                      {bulkPreview.map((mod, i) => (
                        <div key={i} className="flex items-center justify-between px-4 py-3">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500">{mod.module_type || 'unknown'} &middot; {mod.difficulty || 'easy'} &middot; {mod.duration_minutes || 10} min</p>
                          </div>
                          <span className={'text-xs px-2 py-0.5 rounded-full ' + (mod.is_active !== false ? 'bg-green-400/10 text-green-400' : 'bg-gray-500/10 text-gray-400')}>{mod.is_active !== false ? 'Active' : 'Inactive'}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  <Button onClick={handleBulkImport} disabled={!bulkPreview || bulkPreview.length === 0 || bulkUploading} className="bg-[#2979FF] hover:bg-[#2962FF] text-white px-6">
                    <Upload className="w-4 h-4 mr-2" />{bulkUploading ? 'Importing...' : ('Import ' + (bulkPreview ? bulkPreview.length + ' Module' + (bulkPreview.length !== 1 ? 's' : '') : 'Modules'))}
                  </Button>
                  {(bulkJson || bulkPreview) && (<button onClick={() => { setBulkJson(''); setBulkPreview(null); setBulkError(''); }} className="text-sm text-gray-500 hover:text-gray-300 transition-colors">Clear</button>)}
                </div>
              </CardContent>
            </Card>
            <Card className="bg-[#161B22] border-[#30363D]"><CardHeader><CardTitle className="text-sm text-gray-400 font-normal">{modules.length} module{modules.length !== 1 ? 's' : ''} currently in the system</CardTitle></CardHeader></Card>
          </div>
        )}
