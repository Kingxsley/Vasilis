...
# ============== SITEMAP ENDPOINT ==============
# Legacy sitemap endpoint - now the primary
@api_router.get("/sitemap.xml", response_class=Response)
async def generate_sitemap():
    ...
    try:
        ...
        for page in cms_pages:
            if page.get("slug"):
                lastmod = page.get("updated_at", "")[:10] if page.get("updated_at") else ""
                xml_parts.append(f"""  <url>
    ...
    {f'<lastmod>{lastmod}</lastmod>' if lastmod else ''}
        ...
        logger.warning(f"Failed to fetch CMS pages for sitemap: {e}")
