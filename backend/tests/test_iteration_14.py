...
class TestCMSTilesInput:
    ...
    def test_cms_tile_handles_empty_name(self, authenticated_client):
        """Test that CMS tile creation rejects empty name"""
        response = authenticated_client.post(f"{BASE_URL}/api/cms-tiles", json={
            "name": "",
            "icon": "FileText"
        })
        # Backend now validates and rejects empty names
        assert response.status_code == 400
...
class TestFormSubmissions:
    ...
    def test_inquiries_endpoint_exists(self, authenticated_client):
        """Test inquiries endpoint exists and returns data"""
        response = authenticated_client.get(f"{BASE_URL}/api/inquiries")
        assert response.status_code == 200
        data = response.json()
        # Should return inquiries list with pagination info
        assert "inquiries" in data or isinstance(data, list)
